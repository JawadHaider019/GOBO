...
const BestSelling = () => {
  ...
  // Filter to get only bus and living items that are best selling
  const bestSellingItems = listings.filter(item => 
    (item.category === 'BUS' || item.category === 'LIVING') && item.isBestSelling === true
  );

  // Use filtered best selling items
  const items = bestSellingItems;
        ...
        <div className="md:hidden space-y-6">
          ...
          {items.length > 0 && (
            <p className="text-center text-gray-400 text-sm pt-4">
              Showing all {items.length} best selling experiences
            </p>
          )}
        </div>
      </div>
